...
/**
 * Every sticker of the corner and edge the F2L case is about, wherever the two
 * currently are. A picture of an F2L case is really a picture of these six
 * stickers; leaving the rest of the cube in colour buries them, which is why
 * the diagram uses this to decide what to keep bright.
 */
export function pairFacelets(cube: CubieCube): number[] {
  const cornerSlot = cube.cp.indexOf(PAIR_CORNER);
  const edgeSlot = cube.ep.indexOf(PAIR_EDGE);
  return [
    ...(cornerSlot >= 0 ? CORNER_FACELETS[cornerSlot] : []),
    ...(edgeSlot >= 0 ? EDGE_FACELETS[edgeSlot] : []),
  ];
}
