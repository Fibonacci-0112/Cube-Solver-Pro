...
/**
 * The four corners of a sticker's square, in order around it.
 *
 * Stickers sit two units apart in this coordinate system, so a sticker's own
 * half-width is one unit and its corners are the centre plus or minus one step
 * along each of the face's two in-plane directions. `inset` shrinks the square
 * towards its centre to leave the gap a drawing needs between stickers.
 *
 * Drawing a cube in perspective needs the outline of each sticker rather than
 * just its centre, and deriving it here — from the same normals and directions
 * the turns use — is what keeps a picture of a cube honest about the cube.
 */
export function stickerCorners(
  ...
  const centre = stickerPosition(n, face, row, col);
  const rd = ROW_DIR[face];
  const cd = COL_DIR[face];
  const reach = 1 - inset;
  return [
    ...
    const dc = c * reach;
    return [
      centre[0] + dr * rd[0] + dc * cd[0],
      centre[1] + dr * rd[1] + dc * cd[1],
      centre[2] + dr * rd[2] + dc * cd[2],
    ] as const;
  });
}

/** Outward normal of a face, as a unit vector. */
export const faceNormal = (face: Face): Vec => NORMAL[face];
