...
import { platform, useApp } from '../state/app';
...
export function SettingsPage() {
  ...
  const onExport = async () => {
    ...
    setMessage(
      outcome === 'shared'
        ? 'Sent to the app you picked. Keep it somewhere safe — it is your only backup.'
        : 'Exported. Keep that file somewhere safe — it is your only backup.',
    );
  };

  const onImport = async () => {
    const text = await platform().files.open('application/json,.json,.txt');
    if (text === null) return;
    ...
    try {
      const result = await importData(text, replace ? 'replace' : 'merge');
      ...
      <div className="stack">
        ...
        <section className="card">
          ...
          <div className="row">
            ...
            <button onClick={() => void onImport()}>Import a file</button>
          </div>
