...
import { openStore, type ImportMode, type Store } from '../db/store';
import { DEFAULT_SETTINGS, emptyTrainerStat, type Settings, type TrainerStat } from '../db/types';
import { newId, parseImport, type ImportResult } from '../db/transfer';
...
const db = (): Store => {
  if (!store) throw new Error('the local store is not open yet');
  return store;
};
